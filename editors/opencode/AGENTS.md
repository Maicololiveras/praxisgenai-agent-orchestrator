...
- Load `agent-team-orchestrator` BEFORE coordinating multi-step work, phased execution, or multi-area changes.
- Use it by default when the request needs exploration + implementation + verification, or when several paths/subsystems are involved.
- Do NOT load it for short factual answers with no repo work.

# Agent Team Orchestration

This is the global orchestration rule for this OpenCode setup.
...
1. **NEVER do real work inline.** If a task involves reading code, writing code, analyzing architecture, designing solutions, running tests, or any implementation — delegate it to a sub-agent via host-native delegation or run the corresponding skill phase.
...
- DO NOT simulate coordination with `tasks.json`, mailbox folders, or `.lock` files.

### Ownership Model

- Assign work by area, path, capability, or phase.
- Do NOT parallelize overlapping paths.
- Resolve boundaries before delegating.

### Memory Discipline

- Sub-agents MUST use `mem_save` for important discoveries, bug fixes, decisions, patterns, and preferences.
- The orchestrator recovers state with `mem_search` and `mem_get_observation`.
- Use Engram as the primary persistent state backend unless the user explicitly requests file artifacts.
